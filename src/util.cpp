...
const char * const BITCOIN_CONF_FILENAME = "speed.conf";
const char * const BITCOIN_PID_FILENAME = "speedd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "speed" is a composite category enabling all Speed-related debug output
            if(ptrCategory->count(string("speed"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\SpeedCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\SpeedCore
    // Mac: ~/Library/Application Support/SpeedCore
    // Unix: ~/.speed
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "SpeedCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/SpeedCore";
#else
    // Unix
    return pathRet / ".speed";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty speed.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override speed.conf
